import torch
from Semantic_Segmentation_Street_Scenes.backbone.mix_transformer import mit_b4
...
class Teacher(nn.Module):
    ...
    def forward(self, rgb, t):
        ...
        fusion1 = self.fusion1(rgb1, t1, NO=1)
        fusion2 = self.fusion2(rgb2, t2, NO=2)
        fusion3 = self.fusion3(rgb3, t3, NO=3)
        fuison4 = self.fusion4(rgb4, t4, NO=4)
        fusion = [fusion1, fusion2, fusion3, fuison4]
        # for i in fusion:
        #     print(i.shape)
...
class Fusion(nn.Module):
    def __init__(self, inplanes, H, W):
        ...
        self.conv = BasicConv2d(inplanes, 64, 3, 1, 1)
        self.pool = nn.AdaptiveAvgPool2d(output_size=(H // 2, W // 2))
    def forward(self, rgb, t, NO):
        fusion = rgb + t
        if NO == 4:
            out = self.conv(fusion)
        else:
            out = self.conv(self.pool(fusion))
        return out
