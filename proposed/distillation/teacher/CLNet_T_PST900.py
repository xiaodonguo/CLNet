...
class Teacher(nn.Module):
    def __init__(self, distillation=False):
        ...
        if self.training:
            self.rgb.load_state_dict(torch.load(
                "/home/guoxiaodong/code/seg/Semantic_Segmentation_Street_Scenes/backbone/SegFormer_master/weight/mit_b4.pth"),
                                     strict=False)
            self.t.load_state_dict(torch.load(
                "/home/guoxiaodong/code/seg/Semantic_Segmentation_Street_Scenes/backbone/SegFormer_master/weight/mit_b4.pth"),
                                   strict=False)
        self.fusion1 = Fusion(64, 160, 320)
        self.fusion2 = Fusion(128, 80, 160)
        self.fusion3 = Fusion(320, 40, 80)
        self.fusion4 = Fusion(512, 20, 40)
        ...
        self.cbl_out = nn.Sequential(
                                   ...
                                   nn.BatchNorm2d(64),
                                   nn.ReLU(),
                                   nn.Conv2d(in_channels=64, out_channels=5, kernel_size=3, stride=1, padding=1),
                                   )
    ...
    def forward(self, rgb, t):
        ...
        if self.KD ==True:
            ...
            sem = torch.cat((fusion3, fusion4), dim=1)
            return bound1_out, bound2_out, bound3_out, binary1_out, binary2_out, binary3_out, semantic_out, f, sem
        else:
...
if __name__ == '__main__':
    ...
    rgb_input = torch.randn(2, 3, 640, 1280)
    t_input = torch.randn(2, 3, 640, 1280)
    model = Teacher(distillation=True)
