...
class Student(nn.Module):
    def __init__(self, distillation=False):
        ...
        if self.KD == True:
            ...
            # self.aspp1 = ASPP(inchanel=64)
            # self.aspp2 = ASPP(inchanel=64)
            # self.aspp3 = ASPP(inchanel=64)
            # self.aspp4 = ASPP(inchanel=64)
            # self.aspp4 = ASPP(inchanel=64)
    def forward(self, rgb, t):
        ...
        if self.KD ==True:
            ...
            f2 = self.up2(fusion2)
            f = torch.cat((fusion1, f2), dim=1)
            # semantic similarity KD
            sem = torch.cat((fusion4, fusion3), dim=1)
            con = [semantic1, semantic2, semantic3]
            return bound1_out, bound2_out, bound3_out, binary1_out, binary2_out, binary3_out, semantic_out, f, sem, con
        else:
...
class ASPP(nn.Module):
    def __init__(self, inchanel):
        ...
        self.conv1 = DSC(inchannels=inchanel, outchannels=inchanel // 4, kenelsize=3, padding=3, dilation=3)
        self.conv2 = DSC(inchannels=inchanel, outchannels=inchanel // 4, kenelsize=3, padding=5, dilation=5)
        self.conv3 = DSC(inchannels=inchanel, outchannels=inchanel // 4, kenelsize=3, padding=7, dilation=7)
        self.conv4 = DSC(inchannels=inchanel, outchannels=inchanel // 4, kenelsize=3, padding=9, dilation=9)
